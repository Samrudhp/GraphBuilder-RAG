...
He developed the theory of relativity and won the Nobel Prize in Physics in 1921."""
        ...
        print(f"📝 Extracting from text (may take 15-30 seconds)...")
        ...
        if candidates:
            ...
            print(f"   ℹ  This is not a failure - extraction depends on LLM reasoning")
...
async def test_validation_wikipedia():
    ...
    try:
        ...
        print(f"   ℹ  Full validation test requires schema fix (bool vs float)")
...
async def test_entity_resolution_matching():
    ...
    try:
        ...
        if unique_ids < len(entities):
            ...
            print(f"   ℹ All entities treated as unique (similarity < 0.85)")
...
async def test_fusion_deduplication():
    ...
    try:
        ...
        print(f"   ℹ  Full fusion test requires ValidatedTriple schema setup")
...
async def test_embedding_similarity():
    ...
    try:
        ...
        if sim_similar > sim_different:
            print(f"   ✓ Similarity scores make sense!")
